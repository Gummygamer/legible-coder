...
The node and replan budgets are not flat constants; each is sized from the graph the planner actually drafted, since a fixed cap either starves a large plan or is never reached by a small one. The node budget grants one full traversal of the drafted graph plus 3 extra passes (iterative self-repair studies find gains saturate after ~3 attempts, e.g. ["Refining Over Resampling: Test-Time Self-Correction for LLM Reasoning", arXiv:2608.05643](https://arxiv.org/abs/2608.05643)), clamped to at least 12 and at most 200 nodes; every replan re-derives and can only grow this budget from the new graph, so a turn that keeps discovering more work keeps earning more room. The replan budget is 3 (the same self-correction saturation point) plus one redraft per choice node already in the graph, clamped to at most `2 * 3 + 2 = 8`. Both are logged when the graph is drafted, and every replan prompt and every Jev choice request is also told the remaining budget explicitly, since agents that are told their remaining budget reach the same accuracy on a fraction of the tool calls a budget-blind agent needs (["Budget-Aware Tool-Use Enables Effective Agent Scaling", arXiv:2511.17006](https://arxiv.org/abs/2511.17006)). Override either with `LEGIBLE_CODER_JEV_MAX_NODES` / `LEGIBLE_CODER_JEV_MAX_REPLANS` to pin a fixed number instead. `LEGIBLE_CODER_JEV_TEST_TIMEOUT=15` bounds each `test_script` run. The turn's final output is the last tool or test result once the graph reaches an empty `next`/`goto`. This is deliberately a separate experimental backend; unset `LEGIBLE_CODER_JEV_MODE` to use the normal provider chain.
...
| `LEGIBLE_CODER_JEV_MAX_NODES` | No | Maximum decision-graph nodes executed in one Jev turn. Default: dynamic — one traversal of the drafted graph plus 3 repair passes, clamped to `[12, 200]`; grows on replan, never shrinks |
| `LEGIBLE_CODER_JEV_MAX_REPLANS` | No | Maximum whole-graph regenerations Jev may trigger in one turn. Default: dynamic — 3 plus one per choice node in the graph, clamped to `[3, 8]` |
